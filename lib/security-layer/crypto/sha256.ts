/**
 * Native TS implementation of SHA-256
 * Used as part of the STL-C³T Secure Transaction Layer
 */
...
export function sha256(ascii: string): string {
  ...
  for (i = 0; i < words.length; i += 16) {
    ...
    for (j = 0; j < 64; j++) {
      let s0: number, s1: number, ch: number, maj: number, temp1: number, temp2: number;
      if (j < 16) {
        w[j] = w[j] | 0;
      } else {
      ...
      s1 = rotr(hash[4], 6) ^ rotr(hash[4], 11) ^ rotr(hash[4], 25);
      ch = (hash[4] & hash[5]) ^ (~hash[4] & hash[6]);
      temp1 = (hash[7] + s1 + ch + k[j] + w[j]) | 0;
      s0 = rotr(hash[0], 2) ^ rotr(hash[0], 13) ^ rotr(hash[0], 22);
